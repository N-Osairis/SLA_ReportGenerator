...
class creatWord :
  ...
  def __init__(self,Month1,Year1,tableList,outputadd,clientname,graph_no,SLA,RD):
  ...
  def creatingfunction_word(self):
   ...
   paragraph.text = "N’osairis Technology Solutions Sdn Bhd \n Unit 9-6, Level 9, Tower B, Vertical Business Suite 2, Avenue 3, Bangsar South, No. 8, Jalan Kerinchi, 59200 Kuala Lumpur."
   ...
   # H1.add_run( 'N’OSAIRIS TECHNOLOGY SOLUTIONS CONFIDENTIAL', 1)
   H1.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
   H1_run=H1.add_run('N’OSAIRIS TECHNOLOGY SOLUTIONS                CONFIDENTIAL')
   H1_run.bold = True
   ...
   if "PMP" in self.cliname:
       logo = document.add_picture('{}\\{}.png'.format(self.rd, self.cliname), width=Inches(4.7), height=Inches(1.1))
   elif "FJB" in self.cliname:
        logo=document.add_picture('{}\\{}.png'.format(self.rd,self.cliname), width=Inches(4.7),height=Inches(1.1))
   elif "VINX-EMONEY" in self.cliname or "VINX-WAN" in self.cliname:
       logo = document.add_picture('{}\\{}.png'.format(self.rd, self.cliname), width=Inches(3.1), height=Inches(1.6))
  ...
  #creating section breack -----------------------
   ...
   # document.add_picture('IMGoutput\\1.png', width=Inches(1.25))
  #  records = (
  #     (3, '101', 'Spam'),
  #     (7, '422', 'Eggs'),
  ...
  # )
   ...
   if "VINX-WAN" in self.cliname:
    ...
    tablehead = ['NO', 'Site Name', 'Connectivity','Business Hours \n Minutes', 'WAN \n Downtime Minutes','Customer \n Downtime Minutes',
                 'Total Downtime Minutes\n (WAN+Customer)', 'WAN\nAvailability\nPercentage']

   else:
    table = document.add_table(rows=1, cols=9)
    tablehead = ['NO', 'Site Name', 'Connectivity', 'From Date', 'To Date', 'Exact Site Uptime (mins)',
   ...
   # tablehead=list(self.tableList.columns)
   if "VINX-WAN" in self.cliname:
    y=0
    while y<=7:
       # if y<2:
       ...
       y+=1
    count=1
    for x in    self.tableList.sort_values(by='Site Name').index:
     ...
     row_cells[1].text = str(self.tableList['Site Name'][x])
     # row_cells[2].text = str(self.tableList['TNS Router ID'][x])
     row_cells[2].text = str(self.tableList['Connectivity'][x])
     row_cells[3].text = str(self.tableList['Total Availability Time-Business Hours (mins)'][x])
     row_cells[4].text = str(self.tableList['Total downtime_WAN'][x])
     # row_cells[5].text = str(self.tableList['Exact Site Uptime (mins)'][x])
     row_cells[5].text = str(self.tableList['Customer Downtime (mins)'][x])
     row_cells[6].text = str(self.tableList['Total Downtime(wan+customer)(mins)'][x])
     row_cells[7].text = str(self.tableList['Uptime (%)'][x])
     ...
     while y <= 7:
         ...
         font.size = Pt(9)  # font size = 30
         font.name = 'Calibri (Body)'
         if float(self.tableList['Uptime (%)'][x])<self.slaValue:
       ...
       for x in self.tableList.sort_values(by='Site Name').index:
           ...
           # row_cells[2].text = str(self.tableList['TNS Router ID'][x])
           row_cells[2].text = str(self.tableList['Connectivity'][x])
           row_cells[3].text = str(self.tableList['From Date'][x])
           row_cells[4].text = str(self.tableList['To Date'][x])
           row_cells[5].text = str(self.tableList['Exact Site Uptime (mins)'][x])
           row_cells[6].text = str(self.tableList['Total Availability Time-Business Hours (mins)'][x])
           row_cells[7].text = str(self.tableList['Total downtime_WAN'][x])
           row_cells[8].text = str(self.tableList['Uptime (%)'][x])
   ...
   imgcount=1
   while(imgcount<=self.NoOfgraph):
        document.add_picture('{}\\{}.png'.format(self.outputadd,imgcount), width=Inches(8.2),height=Inches(2.8))
        last_paragraph = document.paragraphs[-1]
        last_paragraph.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        last_paragraph_format = last_paragraph.paragraph_format
