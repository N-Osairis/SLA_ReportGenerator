...
if  "PMP" in client_name:
    ...
    SLA = 99.97  # fjb=99.5 vinx=99.7 pmp=99.97
    # 0---->N0  1---->Site Name  6---->TNS Router ID   8---->Wired ISP   15---->SIM ISP  20---->SIM ISP
    MasterCo = [0, 1, 6, 8, 15, 20]
    # 0---->N0  1---->ticket   5---->site    9---->service impact start  10---->service impact end  11---->service downtime  14---->ROOT CAUSE
...
if  "FJB" in client_name:
    ...
    SLA = 99.97  # fjb=99.5 vinx=99.7 pmp=99.97
    # 0---->N0  1---->site code   2---->site name   3---->store type   4---->region  12---->primary line   12----->secondary line
    MasterCo = [0, 1, 2, 3, 4, 12, 18]
    # 0---->N0  1---->ticket   4---->site    8---->service impact start  9---->service impact end  10---->service downtime  12---->ROOT CAUSE
...
if  "VINX-EMONEY" in client_name:
    ...
    # 0---->N0  1---->ticket   5---->site    9---->service impact start  10---->service impact end  11---->service downtime  13---->ROOT CAUSE
...
if  "VINX-WAN" in client_name:
    ...
    SLA=99.7 # pmp=99.97
    # 0---->N0  1---->site code   2---->site name   3---->store type   6---->region  10---->primary line   16----->secondary line
...
def autolabel(ax,rects):
    ...
    for rect in rects:
        ...
        ax.annotate('{}'.format(height),
                    ...
                    ha='center', va='bottom')
def make_multiline(list):
    outputlist=[]
    for st in list:
     outputlist.append(st.replace('-', '\n'))
    tlist= tuple(outputlist)
    return  tlist
def checkDicisExist(Dict, listD):
    output1 = False
    ind=None
    if len(listD) > 0:
        for Indx ,i in enumerate (listD):
            if Dict['SITE'] in i[0]:
                output1 = True
                ind=Indx
    return [output1,ind]
def combineresult(lD1,lD2):
    output=[]
    for i in lD1 :
        for j in lD2 :
            if i['SITE'] == j['SITE'] :
                CI=checkDicisExist(i ,output)
                if CI[0]==False :
                 output.append([i['SITE'],i['value'],j['value']])
                elif CI[0]==True :
                    print(CI[1])
                    output[CI[1]]=[i['SITE'], i['value'], j['value']]
            else:
                CI = checkDicisExist(i, output)
                if CI[0]==False :
                    output.append([i['SITE'], i['value'], 0])
                CI = checkDicisExist(j, output)
                if CI[0] == False:
...
Incident_Row_data=opencsv_specifcColumn(IncidentSheet,IncidentCol)
Master_Row_data=opencsv_specifcColumn(MasterSheet,MasterCo)
# client name may be "PMP", "FJB", "VINX"

if  "VINX-EMONEY" in client_name:
  mdf=pd.DataFrame(Master_Row_data,columns=['NO','Site Code','Site Name','Store Type','Region','primary','secondary','secondary 3G'])
elif  "FJB" in client_name or "VINX-WAN" in client_name:
  mdf=pd.DataFrame(Master_Row_data,columns=['NO','Site Code','Site Name','Store Type','Region','primary','secondary'])
elif "PMP" in client_name :
    mdf = pd.DataFrame(Master_Row_data,columns=['NO', 'Site Name', 'Router ID', 'primary','secondary','secondary 3G'])
...
idf=pd.DataFrame(Incident_Row_data,columns=['NO','TICKET','SITE','SERVICE-IMPACT START','SERVICE-IMPACT END','SERVICE DOWNTIME','ROOT CAUSE'])
...
for x in idf.index:
    if  idf['SERVICE-IMPACT START'][x]!="" and idf['SERVICE-IMPACT END'][x]!="" :
        if  "WAN" in idf['ROOT CAUSE'][x] or "wan" in idf['ROOT CAUSE'][x]:
            totalmin= MinCalculate(idf['SERVICE DOWNTIME'][x])
            print("{} >>>>{}-----------{}-----{}".format(x, idf['SITE'][x], idf['SERVICE DOWNTIME'][x], totalmin))
            incidentlist.append({'NO':idf['NO'][x],'TICKET':idf['TICKET'][x],'SITE':findfjbname(idf['SITE'][x],client_name),'SERVICE-IMPACT START':idf['SERVICE-IMPACT START'][x],'SERVICE-IMPACT END':idf['SERVICE-IMPACT END'][x], 'SERVICE DOWNTIME':idf['SERVICE DOWNTIME'][x],'ROOT CAUSE': idf['ROOT CAUSE'][x],'Total DownTime_min':totalmin})
        else :
            totalmin= MinCalculate(idf['SERVICE DOWNTIME'][x])
            print("{} >>>>{}-----------{}-----{}".format(x, idf['SITE'][x], idf['SERVICE DOWNTIME'][x], totalmin))
            incidentlistCUS.append({'NO':idf['NO'][x],'TICKET':idf['TICKET'][x],'SITE':findfjbname(idf['SITE'][x],client_name),'SERVICE-IMPACT START':idf['SERVICE-IMPACT START'][x],'SERVICE-IMPACT END':idf['SERVICE-IMPACT END'][x], 'SERVICE DOWNTIME':idf['SERVICE DOWNTIME'][x],'ROOT CAUSE': idf['ROOT CAUSE'][x],'Total DownTime_min':totalmin})
...
finalresult=combineresult(result,resultCUS)
